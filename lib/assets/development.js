...
module.exports = function setupDevelopmentAssets(options) {
    ...
    var addJavascriptTag = addTagFactory(manifest, options.assetsBaseURI, 'javascripts', 'script', 'src', {
        'type': 'text/javascript',
    }, utils.html.OPENCLOSE);

    var addStylesheetTag = addTagFactory(manifest, options.assetsBaseURI, 'stylesheets', 'link', 'href', {
        'rel': 'stylesheet',
        'type': 'text/css',
...
function addTagFactory(manifest, assetsBaseURI, type, tag, srcAttr, attributes, opts) {
    ...
    return function add_tag(file, options) {
        ...
        files.forEach(function(file) {
            ...
            attrs[srcAttr] = assetsBaseURI + '/' + type + '/' + file; // TODO: Use something like pathM.join...
